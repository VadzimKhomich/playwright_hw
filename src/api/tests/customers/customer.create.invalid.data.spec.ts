...
test.describe("[API] [SalesPortal] [Create customer with invalid parametrs]", () => {
  createCustomerInvaliData.forEach(({ testName, customer }) => {
    test(testName, async ({ signInController, customerController }) => {
      ...
      expect.soft(customerResponse.body.ErrorMessage).toBe('Incorrect request body')
    });
  });

  test("Imposible to create customer with existing email", async({ signInController, customerController }) => {
    const loginResponse = await signInController.signIn({
      username: USER_LOGIN,
      password: USER_PASSWORD,
    });
    ...
    const customerResponse = await customerController.create(customer, token)
    const customerDuplicate = genereateCustomerData({email: customerResponse.body.Customer.email})
    const customerResponseDuplicate = await customerController.create(customerDuplicate, token)
    expect.soft(customerResponseDuplicate.status).toBe(STATUS_CODES.CONFLICT)
    expect(customerResponseDuplicate.body.ErrorMessage).toBe(`Customer with email '${customerResponse.body.Customer.email}' already exists`)
